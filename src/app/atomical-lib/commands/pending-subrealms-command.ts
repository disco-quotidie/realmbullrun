...
export class PendingSubrealmsCommand implements CommandInterface {
  ...
  async run(pushInfo: Function): Promise<any> {
    ...
    for (let i = 0; i < pending_awaiting_payment.length; i++) {
      ...
      const expectedPaymentOutputsMap = element['applicable_rule']['matched_rule']['o'];
      const paymentOutputs: any= [];

      const outpoint = compactIdToOutpoint(element['atomical_id']);
      ...
      statusReturn.request_subrealm.pending_awaiting_payment[i]['receipt_id'] = paymentReceipt.toString('hex')
  ...
  makePrettyMenu(statusReturn: any) {
  ...
  async promptSubrealmSelection(pendingSubrealms: any): Promise<{ atomicalId: string, value: number } | any> {
